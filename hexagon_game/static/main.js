...
function boardLocalKey(key) {
    return `hexagon_game_${BOARD_ID}_${key}`;
}
...
// 修改所有fetch调用为apiFetch，并本地存储key加board_id
async function makeMove(p1, p2) {
    try {
        const response = await apiFetch('/api/move', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ p1, p2 }) });
        if (!response.ok) { const data = await response.json(); alert('错误: ' + data.error); }
        await fetchGameState();
    } catch (error) { console.error("请求失败:", error); }
}
async function selectMyColor(color) {
    sessionStorage.setItem(boardLocalKey('myColor'), color);
    await apiFetch('/api/select_color', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ color }) });
    myColor = color;
    await fetchGameState();
}
async function selectFollowColor(color) {
    sessionStorage.setItem(boardLocalKey('followColor'), color);
    followColor = color;
    updateUI();
}
...
async function fetchGameState() { 
    ...
    try { 
        ...
        // 直接使用标准格式，同时保持向后兼容
        const boardChanged = JSON.stringify(gameState) !== JSON.stringify(data);
        gameState = data; 
        
        if (data.my_color) { 
            myColor = data.my_color; 
            sessionStorage.setItem(boardLocalKey('myColor'), myColor); 
        } else {
            // 兼容刷新后自动恢复颜色
            const saved = sessionStorage.getItem(boardLocalKey('myColor'));
            if (saved) myColor = saved;
        }
        
        // 跟随色本地恢复
        const savedFollow = sessionStorage.getItem(boardLocalKey('followColor'));
        if (savedFollow) followColor = savedFollow;
...
function startHeartbeat() {
    function send(action='inc') {
        apiFetch('/api/heartbeat?action='+action, { method: 'POST' });
    }
    send('inc');
    heartbeatTimer = setInterval(()=>send('inc'), 20000);
...
async function init() {
    if (!sessionStorage.getItem(boardLocalKey('_color_init'))) {
        sessionStorage.removeItem(boardLocalKey('myColor'));
        sessionStorage.removeItem(boardLocalKey('followColor'));
        sessionStorage.setItem(boardLocalKey('_color_init'), '1');
        myColor = null;
        followColor = null;
    }
    resetButton.addEventListener('click', async () => { if(confirm("确定要重置整个棋盘吗？所有人的进度都将丢失！")) { await apiFetch('/api/reset', { method: 'POST' }); await fetchGameState(); } });
    // 【新功能】 为悔棋按钮绑定事件
